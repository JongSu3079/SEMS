...
<body>
	<div id="wrap">
		<!-- 전체메뉴 -->
		<%@ include file="/pub/common_syspoc/cm/CM0301.jsp"%>
		<!-- //전체메뉴 -->
		
		<!-- 공통 -->
		<%@ include file="/pub/common_syspoc/vm/VM0101.jsp"%>
		<!-- //공통 -->
	</div>
	<!-- //wrap -->
	...
	<div id="air_hit_popup" class="wrap_popup">
		<!-- 냉난방기 제어 -->
		<%@ include file="/pub/common_theme/vm/VM0102.jsp"%>
		<!-- //냉난방기 제어 -->
	</div>
	<!-- //wrap_popup -->
	...
	<script>
		...
		$.fn.tabs = function() {
			...
			function set_view(index) {
				$.each(list, function(key, value) {
					var view = $(this).data("link");
				...
				list.eq(index).data("show", "true").attr("data-show", "true");
				list.eq(index).addClass("tab_on").removeClass("tab_off");
				
				list.not(list.eq(index)).data("show", "").attr("data-show", "");
				list.not(list.eq(index)).addClass("tab_off").removeClass("tab_on");
			}
		}
	</script>
</body>
