...
$(function(){
	...
	calendar = new FullCalendar.Calendar(calendarEl, {
		...
		events:function(arg, sucCallback){
			...
			fn_VM1601RetrieveMntncDate(param, sucCallback);
		},
		customButtons: {
			custom1: {
				text: '점포검색',
				click: function() {
					var form = document.movePageVM1602;
					form.action = '/menu/VM1602';
					form.method = "post";
...
function fn_VM1601RetrieveMntncDate(param, successFunc){
	$.ajax({
		url:'/retrieveMntncDate',
		data:param,
		dataType:'json',
		beforeSend:function(){ 
		...
		success:function(response){
			...
			$("#srcDate").val(date)
			param.date = date;
			fn_VM1601TabClick(param);
		}
	})
}

function fn_VM0601RetrieveMntncList(param){
	param.progStatCd = param.tabCd;
	
	$.ajax({
		type : 'POST',
		url:'/retrieveMntncList',
					...
					if(response.total > 0) {
						response.items.forEach( (item,i) => {
							...
							if(param.tabCd === 'S'){	// 처리예정일때만 화면이동
								html +=  '		<a href="#" onclick="fn_VM1601PageMoveVM1603('+(i + Number(param.startRow))+'); return false;" >';
							}
							html +=  '			<span class="detail">';
							if(item.mntncType === 'N'){
								html +=  '				점검유형 : 일반<br/>';
							}else if(item.mntncType === 'C'){
								html +=  '				점검유형 : 커피머신<br/>';
							}
...
function fn_VM1601changeTab(tabCd){
	$("#moreCnt").val( 0 );
...
function fn_VM1601TabClick(param){
	var records 		= "";
	var moreCnt			= $("#moreCnt").val();
	var pagingRowCnt	= $("#pagingRowCnt").val();
	...
	fn_VM0601RetrieveMntncList(param);
}

//더보기 클릭
function fn_VM1601showMoreList() {
	var moreCnt = $("#moreCnt").val();
...
function fn_VM1601PageMoveVM1603(index){
	var paramData = returnData[index];
	
	$("#movePageParam2").val(JSON.stringify(paramData));
	
	var form = document.movePageVM1603;
	form.action = '/menu/VM1603';
	form.method = "post";
	form.enctype = "application/x-www-form-urlencoded";
	form.submit();
}