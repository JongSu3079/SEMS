...
function fn_VM1602Search(){
	$("#moreCnt").val( 0 );
	
	fn_VM1602RetrieveMntntList();
}

function fn_VM1602RetrieveMntntList(){
				...
				if(response.items != null) {
					...
					if( items.length > 0 ){
						items.forEach( (item,i) => {
							...
							html +=  '		<a href="#" onclick="fn_VM1602PageMoveVM1603('+i+'); return false;" >';
...
//더보기 클릭
function fn_VM1602showMoreList() {
	var moreCnt = $("#moreCnt").val();
...
function fn_VM1602PageMoveVM1603(index){
...
function fn_VM1601Redirect(){
	$('#returnFrom').attr("action", "/VM1601Redirect");
	$("#returnFrom").submit();
}