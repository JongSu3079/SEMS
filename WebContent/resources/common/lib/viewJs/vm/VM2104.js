...
// 검색
function fn_VM2104refresh(){
	$("#moreCnt").val( 0 );
	
	fn_VM2104RetrieveAbsStr();
}

function fn_VM2104RetrieveAbsStr(){
			...
			if(response.success){
				...
				if(response.items && response.items.length > 0){
					...
					items.forEach((item,i) => {
						html += '<li>';
						html += '	<a href="#" class="list_text" onclick="fn_VM2104selAbsStr('+i+'); return false;">';
...
//더보기 클릭
function fn_VM2104showMoreList() {
	var moreCnt = $("#moreCnt").val();
...
function fn_VM2104selAbsStr(index){
